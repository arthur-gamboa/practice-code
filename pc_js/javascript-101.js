...
// function startsAndEndWithVowel(str) {
//   var vowels = "aeiou";
//   var strArr = str.split("").toLowerCase();
//   var endArr = str[str.length -1].toLowerCase();
//   if (vowels.indexOf(strArr[0] > -1) && vowels.indexOf(endArr) > -1) {
//     return true;
//   } else {
//     return false;
//   }
...
function onlyPositiveNumbers(num) {
    var storeArr = [];
    for (var i = 0; i < num.length; i++) {
        if (num[i] > -1) {
            storeArr.push(num[i]);
...
function onlyNegativeNumbers(num) {
    var storeArr = [];
    for (var i = 0; i < num.length; i++) {
        if (num[i] < 0) {
            storeArr.push(num[i]);
...
function onlyNegativeEvens(num) {
    var storeArr = [];
    for (var i = 0; i < num.length; i++) {
        if (num[i] % 2 === 0 && num[i] < 0) {
            storeArr.push(num[i]);
...
function shortestString(str) {
    ...
    for (var i = 0; i < str.length; i++) {
        storeArr.push(str[i].length);
    }
    var shortest = storeArr.indexOf(Math.min(...storeArr));
    return str[shortest];
}
// function shortestString(str) {
//   var storeArr = [];
...
function longestString(str) {
    ...
    var longest = storeArr.indexOf(Math.max(...storeArr));
    return str[longest];
}
...
function getUniqueValues(value) {
    ...
    for (var i = 0; i < value.length; i++) {
        if (storeUniques.indexOf(value[i]) === -1) {
            storeUniques.push(value[i]);
        }
    } return storeUniques;
}
// function getUniqueValues(value) {
//   var storeArr = [];
//   for (var i = 0; i < value.length; i++) {
//     if (storeArr.indexOf(value[i]) === -1) {
//       storeArr.push(value[i]);
//     }
//   } return storeArr;
// }
...
function addOneToArray(add) {
    var storeArr = [];
    for (var i = 0; i < add.length; i++) {
        storeArr.push(add[i] + 1);
    }
    return storeArr;
}
